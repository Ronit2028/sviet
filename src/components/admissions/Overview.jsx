...
const Overview = () => {
    ...
    return (
        <div className="py-12 my-10 w-10/12 mx-auto flex flex-wrap lg:flex-nowrap">
            ...
            <div className="py-12 w-full lg:w-1/3 flex flex-col justify-center">
                <p className="text-4xl pb-3 mb-3 font-semibold">
                    <span className="text-[#fea700]">SVGOI</span>
                    <br />
                    Experience
                </p>
                <p className="text-justify">
            ...
            <div className="w-full lg:w-2/3 lg:pl-16">
                {/* Statistics Row 1 */}
                <div className="gap-4 mb-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4">
                    <div className="bg-gray-100/20 rounded-md shadow-md"></div>
                    <div className="bg-gray-100 rounded-md shadow-md"></div>
                    <StatCard value="250+" title="Faculties Available" subtitle="Every Year" />
                    <StatCard value="25+" title="Acres" subtitle="Campus" bg="bg-[#fea700] text-white" />
                </div>

                {/* Statistics Row 2 */}
                <div className="gap-4 mb-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4">
                    <div className="bg-gray-100 rounded-md shadow-md"></div>
                    <StatCard value="25000+" title="Global Alumni" subtitle="Reach" />
                    <StatCard value="1000+" title="International Students" subtitle="Present" bg="bg-[#fea700] text-white" />
                    <StatCard value="25+" title="National & International" subtitle="Awards Ranking" />
                </div>

                {/* Statistics Row 3 */}
                <div className="gap-4 mb-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4">
                    <StatCard value="350+" title="Companies Visit" subtitle="Per Year" />
                    <StatCard value="5000+" title="Placed Students" subtitle="In 250+ Companies" bg="bg-[#fea700] text-white" />
                    <StatCard value="100%" title="Scholarship" subtitle="Programs" />
                    <StatCard value="45 Lacs+" title="Entrepreneurship" subtitle="Funding" />
                </div>
            </div>
        </div>
    );
...
const StatCard = ({ value, title, subtitle, bg = "bg-white text-black" }) => (
    <div className={`${bg} rounded-md p-5 shadow-md flex flex-col justify-center`}>
        <p className="text-4xl mb-3 font-semibold">{value}</p>
        <p className="text-lg font-medium py-1 w-fit mb-2 rounded-sm">{title}</p>
        <p className="font-medium mb-3 text-sm">{subtitle}</p>
    </div>
);
